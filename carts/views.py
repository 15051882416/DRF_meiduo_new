import pickle, json, base64

from django import http
from django.shortcuts import render
from django.views import View
from django_redis import get_redis_connection

from goods.models import SKU
from meiduo_mall.utils.response_code import RETCODE


class CartsView(View):
    """购物车"""

    def post(self, request):
        """购物车商品添加"""
        # 接收请求体中的 sku_id 和count
        json_dict = json.loads(request.body.decode())
        sku_id = json_dict.get("sku_id")
        count = json_dict.get("count")
        selected = json_dict.get("selected", True)

        # 校验数据
        if all([sku_id, count]) is False:
            return http.HttpResponseForbidden("缺少必传参数")
        try:
            sku = SKU.objects.get(id=sku_id)
        except SKU.DoesNotExist:
            return http.HttpResponseForbidden("sku_id不存在")
        # 判断count是不是int类型
        if isinstance(count, int) is False:
            return http.HttpResponseForbidden("count类型有误")

        # 获取当前请求对象中的user
        user = request.user
        # 判断用户是否登录
        if user.is_authenticated:
            # 如果是登录用户，操作redis购物车数据
            # 创建redis连接对象
            redis_conn = get_redis_connection("carts")
            pl = redis_conn.pipeline()
            # 新增购物车数据
            pl.hincrby("carts_%s" % user.id, sku_id, count)
            # 新增勾选状态
            if selected:
                pl.sadd("selected_%s" % user.id, sku_id)
            pl.execute()

            return http.JsonResponse({"code": RETCODE.OK, "errmsg": "添加购物车成功"})

        else:
            # 如果是未登录用户，操作cookie购物车数据
            """
            carts = {
                sku_id_1: {'count': 1, 'selected': True},
                sku_id_2: {'count': 1, 'selected': True},

            }
            'gdasdgfafasdfdasfdsafsd'
            """
            # 先获取cookie中的购物车数据
            cart_str = request.COOKIES.get("carts")
            # 判断是否有cookie购物车数据
            if cart_str:  # 如果有cookie购物车数据，应该将字符串转换成字典
                # 先将字符串转换成bytes类型
                cart_str_bytes = cart_str.encode()
                # 使用base64把字符串bytes转换成密文bytes
                cart_bytes = base64.b64decode(cart_str_bytes)
                # 使用pickle模型把密文bytes转换成字典
                cart_dict = pickle.loads(cart_bytes)
            else:  # 如果cookie中没有购物车数据
                # 准备一个新字典用来装购物车数据
                cart_dict = {}

            # 判断本次添加的购物车商品是否之前已存在，存在做增量计算
            if sku_id in cart_dict:
                origin_count = cart_dict[sku_id]["count"]
                count += origin_count
            # 如果是一个新商品直接添加到字典
            cart_dict[sku_id] = {
                "count": count,
                "selected": selected
            }
            # 把cookie购物车字典转换成字符串
            cart_str = base64.b64encode(pickle.dumps(cart_dict)).decode()
            # 创建响应对象
            response = http.JsonResponse({"code": RETCODE.OK, "errmsg": "添加购物车成功"})
            # 设置cookie
            response.set_cookie("carts", cart_str)
            return response

    def get(self, request):
        """展示购物车"""
        # 判断用户是否登录
        """
            redis格式
            hash: {sku_id_1: 1, sku_id_16: 2}
            set: {sku_id_1}

            cookie格式
             {
                sku_id_1: {'count': 1, 'selected': True},
                sku_id_2: {'count': 1, 'selected': True},

            }
        """
        user = request.user
        if user.is_authenticated:
            # 登录操作redis购物车数据
            # 把redis中hash和set集合数据取出来
            redis_conn = get_redis_connection('carts')
            # 获取redis的hash字典数据: {sku_id: count}
            redis_dict = redis_conn.hgetall('carts_%s' % user.id)
            # 获取set集合数据 {sku_id}
            selected_ids = redis_conn.smembers('selected_%s' % user.id)

            # 把数据格式转换成和cookie购物车数据格式一致,方便后期统一处理
            # 定义一个字典变量,用来装redis购物车的所有数据
            cart_dict = {}
            # 遍历hash字典数据 向cart_dict中添加
            for sku_id_bytes in redis_dict:
                cart_dict[int(sku_id_bytes)] = {
                    'count': int(redis_dict[sku_id_bytes]),
                    'selected': sku_id_bytes in selected_ids
                }

        else:

            # 未登录操作cookie购物车数据
            cart_str = request.COOKIES.get('carts')
            if cart_str:
                cart_dict = pickle.loads(base64.b64decode(cart_str.encode()))
            else:
                # 如果当前没有购物车数据,直接显示购物车
                return render(request, 'cart.html')
                # cart_dict = {}

        # 通过cart_dict中的key(sku_id)获取到sku模型
        sku_qs = SKU.objects.filter(id__in=cart_dict.keys())
        sku_list = []  # 用来包装每一个购物车商品字典数据

        # 将sku模型 和商品的其它数据包装到同一个字典
        for sku_model in sku_qs:
            # 获取当前商品要购物车的数量
            count = cart_dict[sku_model.id]['count']
            sku_list.append(
                {
                    'id': sku_model.id,
                    'name': sku_model.name,
                    'price': str(sku_model.price),  # 单价是Decimal类型前端解析可能会出错所以转换成str
                    'default_image_url': sku_model.default_image.url,
                    'selected': str(cart_dict[sku_model.id]['selected']),
                    'count': count,
                    'amount': str(sku_model.price * count)  # 小计
                }
            )

        return render(request, 'cart.html', {'cart_skus': sku_list})

    def put(self, request):
        """修改更新购物车"""
        # 接收前端传入的数据
        json_dict = json.loads(request.body.decode())
        sku_id = json_dict.get("sku_id")
        count = json_dict.get("count")
        selected = json_dict.get("selected", True)

        if all([sku_id, count]) is False:
            return http.HttpResponseForbidden("缺少必传参数")
        try:
            sku_model = SKU.objects.get(id=sku_id)
        except SKU.DoesNotExist:
            return http.HttpResponseForbidden("sku_id无效")
        try:
            count = int(count)
        except Exception:
            return http.HttpResponseForbidden("类型有误")
        if isinstance(selected, bool) is False:
            return http.HttpResponseForbidden("类型有误")
        user = request.user
        # 判断用户是否登录
        if user.is_authenticated:
            # 如果用户登录操作redis购物车数据
            redis_conn = get_redis_connection("carts")
            pl = redis_conn.pipeline()
            # 修改hash数据
            pl.hset("carts_%s" % user.id, sku_id, count)
            # 修改勾选状态数据
            if selected:
                pl.sadd("selected_%s" % user.id, sku_id)
            else:
                pl.srem("selected_%s" % user.id, sku_id)
            # 包装一个当前修改的商品新数据字典
            cart_sku = {
                "id": sku_model.id,
                "name": sku_model.name,
                "default_image_url": sku_model.default_image.url,
                "count": count,
                "price": str(sku_model.price),
                "amount": str(sku_model.price * count),
                "selected": selected

            }
            pl.execute()
            return http.JsonResponse({"code": RETCODE.OK, "errmsg": "修改购物车数据成功", "cart_sku": cart_sku})

        else:  # 如果用户未登录操作cookie购物车数据
            cart_str = request.COOKIES.get("carts")
            # 判度购物车数据是否存在
            if cart_str:
                cart_dict = pickle.loads(base64.b64decode(cart_str.encode()))
            else:
                return http.JsonResponse({"code": RETCODE.DBERR, "errmsg": "cookie数据没有获取到"})
            # 修改cookie数据
            cart_dict[sku_id] = {
                "count": count,
                "selected": selected
            }
            # 将cart_dict字典转换成字符串
            cart_str = base64.b64encode(pickle.dumps(cart_dict)).decode()

            # 购物车的修改结果展示是由后端将修改后的数据响应到前端，并进行渲染
            # 准备响应数据
            cart_sku = {
                "id": sku_model.id,
                "name": sku_model.name,
                "default_image_url": sku_model.default_image.url,
                "count": count,
                "price": str(sku_model.price),
                "amount": str(sku_model.price * count),
                "selected": selected

            }
            # 创建响应并设置cookie
            response = http.JsonResponse({"code": RETCODE.OK, "errmsg": "修改购物车数据成功", "cart_sku": cart_sku})
            response.set_cookie("carts", cart_str)
            return response

    def delete(self, request):
        """购物车商品删除"""
        json_dict = json.loads(request.body.decode())
        sku_id = json_dict.get("sku_id")
        try:
            sku_model = SKU.objects.get(id=sku_id)
        except SKU.DoesNotExist:
            return http.HttpResponseForbidden("sku_id无效")
        user = request.user
        if user.is_authenticated:
            # 登录用户操作redis购物车数据
            redis_conn = get_redis_connection("carts")
            pl = redis_conn.pipeline()
            # 删除hash数据中sku_id键值对
            pl.hdel("carts_%s" % user.id, sku_id)
            # 将sku_id从set集合中移出
            pl.srem("selected_%s" % user.id, sku_id)
            pl.execute()
            return http.JsonResponse({'code': RETCODE.OK, 'errmsg': "删除购物车成功"})

        else:
            cart_str = request.COOKIES.get("carts")
            # 判断cart_str数据是否存在
            if cart_str:
                cart_dict = pickle.loads(base64.b64decode(cart_str.encode()))
            else:
                # 没获取到提前响应
                return http.JsonResponse({'code': RETCODE.DBERR, 'errmsg': 'cookie数据没获取到'})
            # 删除指定sku_id的键值对
            if sku_id in cart_dict:
                del cart_dict[sku_id]

            response = http.JsonResponse({'code': RETCODE.OK, 'errmsg': "删除购物车成功"})
            # 判断当前字典是为空,如果为空 将cookie删除
            # 判断当前字典是为空目的：看删除的这个sku_id是否是最后一个
            if not cart_dict:
                # 删除cookie, 删除cookie的原理 实现就是在设置cookie把它的过期时间设置为0
                response.delete_cookie("carts")
                return response

            cart_str = base64.b64encode(pickle.dumps(cart_dict)).decode()
            response.set_cookie("carts", cart_str)
            return response


class CartsSelectedAllView(View):
    """购物车全选"""

    def put(self, request):
        """接收前端传入的参数"""
        json_dict = json.loads(request.body.decode())
        selected = json_dict.get("selected")
        # 校验
        if isinstance(selected, bool) is False:
            return http.HttpResponseForbidden("传入的参数类型错误")

        user = request.user
        # 判断用户是否登录
        if user.is_authenticated:
            # 操作redis购物车数据
            redis_conn = get_redis_connection("carts")
            pl = redis_conn.pipeline()
            # 获取hash数据
            redis_dict = redis_conn.hgetall("carts_%s" % user.id)
            # 如果是全选将hash中的所有key添加到set集合中
            if selected:
                pl.sadd("selected_%s" % user.id, *redis_dict.keys())
            else:  # 如果是取消全选将set集合中所有的key全部删除
                # redis_conn.srem("selected_%s" % user.id, *redis_dict.keys())
                pl.delete("selected_%s" % user.id)
            pl.execute()

            # 响应
            return http.JsonResponse({'code': RETCODE.OK, 'errmsg': 'OK'})

        else:
            # 操作cookie购物车数据
            cart_str = request.COOKIES.get("carts")
            # 判断cart_str数据是否存在
            if cart_str:
                cart_dict = pickle.loads(base64.b64decode(cart_str.encode()))
            else:
                return http.HttpResponseForbidden("cookie购物车数据不存在")
            # 遍历cart_dict获取sku_id
            for sku_id in cart_dict:
                cart_dict[sku_id]["selected"] = selected
            # 构建响应，设置cookie
            response = http.JsonResponse({"code": RETCODE.OK, "errmsg": "OK"})
            cart_str = base64.b64encode(pickle.dumps(cart_dict)).decode()
            response.set_cookie("carts", cart_str)
            return response


class CartsSimpleView(View):
    """展示商品页面简单购物车数据"""

    def get(self, request):
        """展示购物车"""
        # 判断用户是否登录
        user = request.user
        if user.is_authenticated:
            # 登录操作redis购物车数据
            # 把redis中hash和set集合数据取出来
            redis_conn = get_redis_connection('carts')
            # 获取redis的hash字典数据: {sku_id: count}
            redis_dict = redis_conn.hgetall('carts_%s' % user.id)
            # 获取set集合数据 {sku_id}
            selected_ids = redis_conn.smembers('selected_%s' % user.id)

            # 把数据格式转换成和cookie购物车数据格式一致,方便后期统一处理
            # 定义一个字典变量,用来装redis购物车的所有数据
            cart_dict = {}
            # 遍历hash字典数据 向cart_dict中添加
            for sku_id_bytes in redis_dict:
                cart_dict[int(sku_id_bytes)] = {
                    'count': int(redis_dict[sku_id_bytes]),
                    'selected': sku_id_bytes in selected_ids
                }

        else:

            # 未登录操作cookie购物车数据
            cart_str = request.COOKIES.get('carts')
            if cart_str:
                cart_dict = pickle.loads(base64.b64decode(cart_str.encode()))
            else:
                # 如果当前没有购物车数据,直接显示购物车
                return render(request, 'cart.html')
                # cart_dict = {}

        # 通过cart_dict中的key(sku_id)获取到sku模型
        sku_qs = SKU.objects.filter(id__in=cart_dict.keys())
        sku_list = []  # 用来包装每一个购物车商品字典数据

        # 将sku模型 和商品的其它数据包装到同一个字典
        for sku_model in sku_qs:
            # 获取当前商品要购物车的数量
            count = cart_dict[sku_model.id]['count']
            sku_list.append(
                {
                    'id': sku_model.id,
                    'name': sku_model.name,
                    'default_image_url': sku_model.default_image.url,
                    'count': count,
                }
            )

        return http.JsonResponse({"code": RETCODE.OK, "errmsg": "ok", "cart_skus": sku_list})
